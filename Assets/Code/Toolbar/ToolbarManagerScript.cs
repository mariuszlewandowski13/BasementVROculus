...
public class ToolbarManagerScript : MonoBehaviour {
    ...
    void Start()
    {
        backButton = transform.Find("buttons").Find("BackButton").gameObject;
        backButton.SetActive(false);
        colorsBar = transform.Find("paski").gameObject;
        colorsBar.SetActive(false);

        CreateToolsObjects();
        FillToolsObjects();

        if (!TutorialScript.tutorialActive /*|| ApplicationStaticData.adminMode*/)
        {
            MainBar();
        }
    ...
    {
        toolsObjects = new ToolsButtons[9];
    }
    ...
    private void FillToolsObjects()
    {
        ...
        toolsObjects[6] = new ToolsButtons(null, giphyIcon, false, true,6, 1);
        toolsObjects[7] = new ToolsButtons(null, googleIcon, false, true,7, 2);
        toolsObjects[8] = new ToolsButtons(null, browserIcon, false, true,8, 3);
    }
    ...
    {
        Color color = colorsBar.transform.Find("panel_color").GetComponent<ToolbarMainColorPickerScript>().color;
        color.a = 1.0f;
        return color;
    }
