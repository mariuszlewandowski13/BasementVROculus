...
public class ToolbarTileRotationScript : MonoBehaviour {
    ...
    private void Move(float Speed)
    {
       ...
       // Debug.Log("Moving");
        ...
        if (ile < 15.0f)
        {
            miniature = transform.Find("ToolbarTile").GetComponent<ToolbarTileMiniatureScript>().GetRightSceneObject();
            if (miniature != null)
            {
        ...
        {
            miniature = transform.Find("ToolbarTile").GetComponent<ToolbarTileMiniatureScript>().GetLeftSceneObject();
    ...
    private void SetAlpha(float angle)
    {
        ...
        if (angle < 30.0f)
        {
            Color color = transform.Find("ToolbarTile").GetComponent<Renderer>().material.color;
            color.a = (angle - 10.0f) / 20.0f;
            transform.Find("ToolbarTile").GetComponent<Renderer>().material.color = color;
            if (transform.Find("ToolbarTile").Find("TileMiniature") != null && transform.Find("ToolbarTile").Find("TileMiniature").GetComponent<Renderer>() != null)
            {
                transform.Find("ToolbarTile").Find("TileMiniature").GetComponent<Renderer>().material.color = color;
            }
        }
