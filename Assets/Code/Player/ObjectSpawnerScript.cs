...
public class ObjectSpawnerScript : MonoBehaviour {
    ...
    public GameObject SpawnObject(SceneObject objectToSpawn, Vector3 newObjectPosition, Quaternion rotation, Vector3 scale, GameObject controller, bool toSave = false)
    {
        ...
        else if (objectToSpawn is VideoObject)
        {
            result = objectSpawner.SpawnVideoObject((VideoObject)objectToSpawn, newObjectPosition, rotation, scale);
        }
        else if (objectToSpawn is BrowserObject)
        {
            result = objectSpawner.SpawnBrowserObject((BrowserObject)objectToSpawn, newObjectPosition, rotation, scale);
        }
        else
        {
            Debug.Log("Nie ma takiej opcji");
        }
    ...
    public GameObject SpawnDuplicateObject(GameObject pattern, GameObject controller)
    {
        ...
        else if (pattern.GetComponent<VideoScript>() != null)
        {
            result = objectSpawner.SpawnVideoDuplicate(pattern);
        }
        else if (pattern.GetComponent<BrowserScript>() != null)
        {
            result = objectSpawner.SpawnBrowserDuplicate(pattern);
        }
        else
        {
            Debug.Log("Nie ma takiej opcji");
    ...
    public void SpawnObjectFromWebBrowser(SceneObject objectToSpawn, Vector3 newObjectPosition, Quaternion rotation, Vector3 scale, GameObject controller, bool toSave)
    {
        GameObject newImgObj = GameObject.Find("Player").GetComponent<ObjectSpawnerScript>().SpawnObject(objectToSpawn, newObjectPosition, rotation, scale, controller, toSave);
        newImgObj.transform.LookAt(GameObject.Find("Player").transform.Find("OVRCameraRig").Find("TrackingSpace").Find("CenterEyeAnchor").transform, Vector3.up);
        newImgObj.transform.Rotate(90.0f, 0.0f, 0.0f);
        newImgObj.AddComponent<DropScript>();
        newImgObj.GetComponent<DropScript>().SetParentObject(controller.GetComponent<ControlObjects>().rotationCenter, controller);
    ...
    {
        GameObject newObject = GameObject.Instantiate(objectToSpawn, objectToSpawn.transform.position, objectToSpawn.transform.rotation, null);
        DestroyImmediate( newObject.GetComponent<TagSpawnScript>());
