...
public class ObjectSpawnerFromGame : ObjectSpawner
{
    ...
    public GameObject SpawnBrowserObject(BrowserObject browser, Vector3 position, Quaternion rotation, Vector3 scale)
    {
        ...
        newObject.transform.gameObject.GetComponent<BrowserScript>().SetBrowserObject(browser);
        newObject.transform.gameObject.GetComponent<BrowserScript>().SetBrowserObjectByNetwork(browser);
        //newObject.transform.gameObject.GetComponent<ZenFulcrum.EmbeddedBrowser.Browser>().firstPageLoaded = true;

        Vector3 posToLookAt = Camera.main.transform.rotation.eulerAngles;
        posToLookAt.x = 0.0f;
        posToLookAt.z = 0.0f;

        newObject.transform.rotation = Quaternion.Euler(posToLookAt);
        AddSetObjectAdditionalFunctions(newObject);
        return newObject;
    }

    public GameObject SpawnBrowserDuplicate(GameObject pattern)
    {
        Object olObject = masterObject.GetComponent<WorldObjects>().browserObject;
        ...
        newObject.tag = tagName;
        newObject.name = "BrowserObject";
        newObject.GetComponent<BrowserScript>().SetBrowserObject(pattern.GetComponent<BrowserScript>().browserObject);
        newObject.GetComponent<BrowserScript>().browserObject.saved = false;

        newObject.transform.gameObject.GetComponent<BrowserScript>().SetBrowserObjectByNetwork(pattern.GetComponent<BrowserScript>().browserObject);
        AddSetObjectAdditionalFunctions(newObject);
    ...
    public GameObject SpawnVideoObject(VideoObject videoObj, Vector3 position, Quaternion rotation, Vector3 scale)
    {
        ...
        newObject.transform.localScale = scale;
        //newObject.transform.Rotate(new Vector3(180.0f, 180.0f, 0.0f));
        newObject.GetComponent<VideoScript>().SetVideoObject(videoObj);
        newObject.GetComponent<VideoScript>().SetVideoObjectByNetwork(videoObj);

        return newObject;
    }

    public GameObject SpawnVideoDuplicate(GameObject pattern)
    {
        Object olObject = masterObject.GetComponent<WorldObjects>().videoObject;
        //GameObject newObject = (GameObject)GameObject.Instantiate(pattern, pattern.transform.position, new Quaternion());
        GameObject newObject = InstantiateGameObject(olObject, pattern.transform.position, pattern.transform.rotation, pattern.transform.localScale);
        ...
        newObject.tag = tagName;
        newObject.name = "VideoObject";
        newObject.GetComponent<VideoScript>().SetVideoObject(pattern.GetComponent<VideoScript>().videoObject);
        newObject.GetComponent<VideoScript>().videoObject.saved = false;
        newObject.GetComponent<VideoScript>().SetVideoObjectByNetwork(pattern.GetComponent<VideoScript>().videoObject);
        //newObject.transform.rotation = pattern.transform.rotation;
    ...
    public void AddSetObjectAdditionalFunctions(GameObject gameObj)
    {
        ...
        if (gameObj.GetComponent<TextScript>() != null)
        {
            ...
            Transform toolsDock = gameObj.transform.Find("ToolsDock");
            ...
            button.transform.Find("button").GetComponent<TextEditButtonScript>().myTextObject = gameObj;
        }

        if (gameObj.GetComponent<BrowserScript>() != null && ApplicationStaticData.CanInteract())
        {
            GameObject olObject = masterObject.GetComponent<ToolsObjects>().searchButton;
            GameObject button = (GameObject)GameObject.Instantiate(olObject, new Vector3(), new Quaternion());
            button.GetComponent<SaveTransformScript>().SetParentObject(gameObj);
            button.transform.Find("button").GetComponent<BrowserButtonScript>().browser = gameObj;

            olObject = masterObject.GetComponent<ToolsObjects>().browserBackButton;
            GameObject button2 = (GameObject)GameObject.Instantiate(olObject, new Vector3(), new Quaternion());
            button2.GetComponent<SaveTransformScript>().SetParentObject(gameObj);
            button2.transform.Find("button").GetComponent<BrowserButtonScript>().browser = gameObj;
