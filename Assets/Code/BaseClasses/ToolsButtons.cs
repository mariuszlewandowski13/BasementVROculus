...
public class ToolsButtons : ToolsObject {
    ...
    public void OnCharAdded(string newChar)
    {
        ...
        else if (newChar == "done")
        {
            ...
            if (sourceForSceneObjects == 1)
            {
                if (taskNumber > 0)
                {
                    GameObject.Find("Player").GetComponent<TasksScript>().UpdateTaskOccurance(taskNumber, searchText);
                }
                SearchGiphy();
    ...
    public void SearchGiphy()
    {
        lock(searchGiphyLock)
        {
            ...
            GameObject.Find("Player").transform.Find("Toolbar").GetComponent<GiphyCustomSearch>().resultReady += GetResultGiphy;
            GameObject.Find("Player").transform.Find("Toolbar").GetComponent<GiphyCustomSearch>().Search(searchText);
        }
    }
    ...
    public void SearchGoogle()
    {
        lock (searchGoogleLock)
        {
            ...
            GameObject.Find("Player").transform.Find("Toolbar").GetComponent<GoogleCustomSearch>().resultReady += GetResultGoogle;
            GameObject.Find("Player").transform.Find("Toolbar").GetComponent<GoogleCustomSearch>().Search(searchText);
        }
    }
    ...
    public void GetResultGiphy()
    {
        lock (searchGiphyLock)
        {
            searchGiphy = false;
            GameObject.Find("Player").transform.Find("Toolbar").GetComponent<GiphyCustomSearch>().resultReady -= GetResultGiphy;
            sceneObjects = GameObject.Find("Player").transform.Find("Toolbar").GetComponent<GiphyCustomSearch>().videos.ToArray();
    ...
    public void GetResultGoogle()
    {
        lock (searchGoogleLock)
        {
            ...
            sceneObjects = GameObject.Find("Player").transform.Find("Toolbar").GetComponent<GoogleCustomSearch>().images.ToArray();
            if (newSceneObjectsList != null)
            {
                newSceneObjectsList();
            }
