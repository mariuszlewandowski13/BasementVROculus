...
public class RemoteDrawingScript : MonoBehaviour {
    ...
    private void DrawPoint(Vector3 point, int number)
    {
        if (PhotonView.Find(number) != null && PhotonView.Find(number).GetComponent<LineScript>().remoteDrawing == true)
        {
            ...
            line.GetComponent<LineRenderer>().positionCount = newPoints.Count;
    ...
    {
        return GameObject.Find("Player").transform.Find("Toolbar").Find("paski").GetComponent<LinesTexturesScript>().lines[num];
    }

    private Material LoadMaterial(int num)
    {
        return GameObject.Find("Player").transform.Find("Toolbar").Find("paski").GetComponent<LinesTexturesScript>().linesMaterials[num];
    }
