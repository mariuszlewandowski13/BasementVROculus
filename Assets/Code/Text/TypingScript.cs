...
public class TypingScript : MonoBehaviour {
    ...
    {
        if (transform.Find("ScaleHandler") != null)
        {
            Vector3 size = GetComponent<BoxCollider>().size;
            Vector3 center = GetComponent<BoxCollider>().center;
            transform.Find("ScaleHandler").transform.localScale = new Vector3(size.x / 10.0f, transform.Find("ScaleHandler").transform.localScale.y, size.y / 10.0f);
            transform.Find("ScaleHandler").localPosition = center;
        }   
    }
