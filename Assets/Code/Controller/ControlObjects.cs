...
public class ControlObjects : MonoBehaviour
{
    ...
    void Update()
    {
            ...
            bool gripUp = OVRInput.GetUp(OVRInput.Button.PrimaryHandTrigger, controller);
    ...
    {
       // Debug.Log("trigger emter: " + collider.name);
        lock (canEnterLock)
        {
            if (canEnter)
            {
    ...
    private void CheckCollidingObjects()
    {
        ...
        {
            pickup.GetComponent<ObjectInteractionScript>().SetCollision(false, gameObject);
            Debug.Log("Deleted pickup: " + pickup.name);
            pickup = null;
