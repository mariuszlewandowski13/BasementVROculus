...
public class ClickMeshBrowserUI : MonoBehaviour, IBrowserUI {
	 ...
	 */
        public static ClickMeshBrowserUI Create(MeshCollider meshCollider, bool logIn = false) {
            ClickMeshBrowserUI ui;
            if (logIn)
            {
                ui = meshCollider.gameObject.AddComponent<ClickMeshBrowserUI>();
            }
            else {
                ui = meshCollider.gameObject.AddComponent<BrowserControllerInteractionScript>();
            }
	...
	protected virtual Ray LookRay {
		get {return GameObject.Find("Monitor").GetComponent<Camera>().ScreenPointToRay(Input.mousePosition); }
	}
	...
	protected void LookOff() {
		if (BrowserCursor != null && mouseWasOver) {
			SetCursor(null);
                //GameObject.Find("Player").transform.FindChild("Controller (right)").FindChild("ControlObject").GetComponent<LineRenderer>().SetVertexCount(0);
            }
		mouseWasOver = false;
	}
