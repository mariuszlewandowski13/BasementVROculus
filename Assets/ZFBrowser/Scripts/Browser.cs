...
public class Browser : MonoBehaviour {
	private static int lastUpdateFrame;
        ...
        public bool firstPageLoaded;
        private bool extraLoading;
...
")]
	...
	protected void Awake() {
        ...
        onNativeReady += id => {
			if (!uiHandlerAssigned) {
				...
				if (meshCollider) UIHandler = ClickMeshBrowserUI.Create(meshCollider, ApplicationLoadScript.loadingLogin);
			}
	...
	protected void OnItemChange(BrowserNative.ChangeType type, string arg1) {
            ...
            //if (type == BrowserNative.ChangeType.CHT_LOAD_FINISHED)
            //{
            //    GetComponent<BrowserScript>().browserObject.url = Url;
            //    SaveManager.SaveGameObject(gameObject);
            //    onloadActions.Add(new Action(InjectJS));
            //    GetComponent<BrowserScript>().SetUrlByNetwork(Url);
            //}
	...
	protected void ProcessCallbacks() {
        ...
        protected void Update()
        {
            ...
            HandleInput();
            //update browser by network
            var urlData = BrowserNative.zfb_getURL(browserId);
            var ret = Marshal.PtrToStringAnsi(urlData);


                if (firstPageLoaded && ret != GetComponent<BrowserScript>().browserObject.url)
                {
                    GetComponent<BrowserScript>().browserObject.url = ret;
                    SaveManager.SaveGameObject(gameObject);
                    onloadActions.Add(new Action(InjectJS));

                    if (GetComponent<PhotonView>().isMine)
                    {
                        GetComponent<BrowserScript>().SetUrlByNetwork(ret);
                    }

                }
                else if (!firstPageLoaded && ret != "" && isHost(new Uri(ret), new Uri( GetComponent<BrowserScript>().browserObject.url)))
                {
                    onloadActions.Add(new Action(InjectJS));
                    firstPageLoaded = true;
                }

                if (!extraLoading && firstPageLoaded && GetComponent<BrowserScript>().browserObject.url == "about:blank")
                {
                    Url = ApplicationStaticData.creatorsPage;
                    extraLoading = true;
                }
        ...
        private bool isHost(Uri ret, Uri browser)
        {
            if (ret.Host == null || ret.Host == "" || browser.Host == null || browser.Host == "") return false;
            if (ret.Segments[0] == browser.Segments[0]) return true;
            else return false;
        }
	...
	public void UpdateCursor() {
        ...
        {
            RegisterFunction("OnActiveElementChange", args => UpdateInput(args[0], args[1], args[2]));
            RegisterFunction("OnStartDragFunction", args => GetImageSrc(args[0]));
            GetComponent<BrowserControllerInteractionScript>().loadOnClick = true;
            //RegisterFunction("OnStopDragFunction", args => RemoveImageSrc());
        }

        private void UpdateInput(JSONNode node, JSONNode innerHTML, JSONNode type)
        {
            input = node;
            if ((!KeyboardHandlerScript.keyboardActive) && ((((string)input.Value == "INPUT") &&  BrowserHelperScript.PossibleInputFieldTypes.Contains((string)type.Value)) || (string)input.Value == "TEXTAREA"))
            {
                GetComponent<BrowserControllerInteractionScript>().InputOn((string)innerHTML.Value);
            }
            else if ((KeyboardHandlerScript.keyboardActive) && ((((string)input.Value != "INPUT" && (string)input.Value != "TEXTAREA")) || (!BrowserHelperScript.PossibleInputFieldTypes.Contains((string)type.Value)))) 
            {
                GetComponent<BrowserControllerInteractionScript>().InputOff();
            }
            //Debug.Log((string)input.Value);
        }

        private void GetImageSrc(JSONNode src)
        {
            imgSrc = (string)src.Value;
            Debug.Log("DragStart");
        }
