	...
	public class ArcTeleporter : MonoBehaviour 
	{
		...
		virtual protected bool IsGoodSpot(RaycastHit hit)
		{
            if (hit.transform == null)
            {
                if (door != null)
                {
                    door.GetComponent<DoorOpenScript>().disableHighlight();
                    door = null;
                }
                return false;
            }
			...
			if (onlyLandOnTag)
			{
                foreach (string tag in tags)
                {
                    ...
                    {
                        if (hitDoor = hit.transform.tag == "door")
                        {
                            if (door != null)
                            {
                                door.GetComponent<DoorOpenScript>().disableHighlight();
                            ...
                            door = hit.transform;
                            door.GetComponent<DoorOpenScript>().highlightRed();
                        }
                        else {
                            if (door != null)
                            {
		...
		{
            if (hitDoor && door != null && door.GetComponent<DoorOpenScript>() != null)
            {
                if (PhotonNetwork.inRoom)
                {
                    PhotonNetwork.Disconnect();
                }
                door.GetComponent<DoorOpenScript>().SetSceneToLoad();
            }
