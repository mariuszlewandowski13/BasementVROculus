   ...
   // }
    ...
    void Update()
    {
        ...
        if (isActiveControllerSet)
        {
            ...
            bool shouldTeleport = OVRInput.GetUp(OVRInput.Button.One, ActiveController);
            bool shouldCancel = OVRInput.GetUp(OVRInput.Button.PrimaryHandTrigger, ActiveController);


            if (shouldTeleport && doorsAreSelected())
            {
                ApplicationStaticData.roomToConnectName = sphere.GetComponent<DoorCollider>().doorCollider.roomName;
                if (PhotonNetwork.inRoom)
                {
                    PhotonNetwork.Disconnect();
                }
                
                sceneToLoad = sphere.GetComponent<DoorCollider>().doorCollider.sceneName;
                loadScene = true;

               // device.TriggerHapticPulse(1000);
            }

            if (shouldTeleport && PositionTeleporterIsSelected())
            {
                sphere.GetComponent<DoorCollider>().changePositionScript.ChangePosition();
                sphere.GetComponent<DoorCollider>().positionCollider = false;
               // device.TriggerHapticPulse(1000);
            }
        ...
        } else
        {
            ...
            int i = 0;
            foreach (OVRInput.Controller obj in Controllers)
            {                           
                if (canControllerTeleport[i])
                {
                    if (OVRInput.GetDown(OVRInput.Button.One, obj))
                    {
                        ...
                        isActiveControllerSet = true;
                        Pointer.transform.parent = ControllersTransform[i];
    ...
    private bool doorsAreSelected()
    {
        if (sphere && sphere.GetComponent<DoorCollider>().collide == true)
        {
            return true;
        }
        else return false;
    ...
    private bool PositionTeleporterIsSelected()
    {
        if (sphere && sphere.GetComponent<DoorCollider>().positionCollider == true)
        {
            return true;
        }
        else return false;
